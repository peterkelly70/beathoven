...
async def track(ctx, track_number: int):
    ...
    if STATUS.NOT_CONNECTED in status:
        ...
        print ("Requested Track: ", track_number)
        new_index = track_number - 1  # Subtract 1 because list indices start at 0
        # Ensure new_index is not less than 0 and not greater than the last index
        new_index = max(0, min(new_index, len(current_playlist['playlist']) - 1))
